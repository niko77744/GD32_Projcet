...
static void led_get_state(led_t *p, led_state *state)
{
    ...
    child_led_t *child = (child_led_t *)p; /* 将传入的指针转换为child_led_t类型 */
...
static void child_led_set(led_t *p, led_state state)
{
    ...
    {
        state = (led_state)!state; /* 如果是低电平有效, 则翻转状态 */
    }
...
static void child_led_toggle(led_t *p)
{
    ...
    if (p->led_base.active_low)
    {
        p->led_base.state = (led_state)!p->led_base.state; /* 如果是低电平有效, 则翻转状态 */
    }
    else
    {
        p->led_base.state = (led_state)!p->led_base.state; /* 如果是高电平有效, 则翻转状态 */
    }
